...
/** Provider/tier diagnostics — only printed when METER_DEBUG is set, so the
 *  fallback plumbing never leaks into the Meter TUI. */
const __fblog = (...a: unknown[]): void => { if (process.env.METER_DEBUG) console.error(...a); };
...
export async function streamWithFallback(
  ...
  if (directProvider && directKey) {
    try {
      __fblog("[fallback] tier 1 (direct key):", requestedModel);
      ...
      __fblog("[fallback] tier 1 (direct) failed:", requestedModel, e.message, (e as any).status, (e as any).code, JSON.stringify((e as any).error ?? "").slice(0, 500));
  ...
  if (bedrockModelId && isBedrockAvailable()) {
    try {
      __fblog("[fallback] tier 2 (bedrock, same model):", requestedModel, "→", bedrockModelId);
      ...
      const e = err as Error;
      __fblog("[fallback] tier 2 (bedrock) failed:", requestedModel, e.message);
  ...
  if (process.env.OPENROUTER_API_KEY) {
    try {
      __fblog("[fallback] tier 3 (openrouter):", requestedModel);
      ...
      const e = err as Error;
      __fblog("[fallback] tier 3 (openrouter) failed:", requestedModel, e.message);
  ...
  for (const candidateModel of candidates) {
    ...
    if (candidateProvider && candidateKey) {
      try {
        __fblog("[fallback] tier 4 (direct, auto-route):", candidateModel);
        ...
        const e = err as Error;
        __fblog("[fallback] tier 4 direct failed:", candidateModel, e.message);
    ...
    if (process.env.OPENROUTER_API_KEY) {
      try {
        __fblog("[fallback] tier 4 (openrouter, auto-route):", candidateModel);
        if (!silent) send({ type: "rerouting", from: requestedModel, to: candidateModel, provider: providerLabel });
        const result = await streamOpenRouter(candidateModel, conversation, tools, send, estimateTokens, totalTokensOut, timeoutMs);
        return { ...result, actualModel: candidateModel, tier: 4 };
      } catch (err) {
        const e = err as Error;
        __fblog("[fallback] tier 4 openrouter failed:", candidateModel, e.message);
        errors.push({ tier: 4, model: candidateModel, error: e.message });
      }
    }
    ...
    if (candidateBedrockId && isBedrockAvailable()) {
      try {
        __fblog("[fallback] tier 4 (bedrock, auto-route):", candidateModel, "→", candidateBedrockId);
        if (!silent) send({ type: "rerouting", from: requestedModel, to: candidateModel, provider: providerLabel });
        const result = await streamBedrock(candidateBedrockId, conversation, tools, send, estimateTokens, totalTokensOut);
        return { ...result, actualModel: candidateModel, tier: 4 };
      } catch (err) {
        const e = err as Error;
        __fblog("[fallback] tier 4 bedrock failed:", candidateModel, e.message);
        errors.push({ tier: 4, model: candidateModel, error: e.message });
      }
    }
  }
  ...
  // All tiers exhausted
  __fblog("[fallback] all tiers failed:", JSON.stringify(errors));
