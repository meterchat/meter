      ...
      {groups.map(({ category, items }) => (
        <div key={category}>
          <button
            ...
            className="flex w-full items-center gap-1.5 py-1.5 px-2 rounded-md hover:bg-white/[0.04] transition-colors"
          >
            <svg
              width="10"
              height="10"
              ...
              className={`shrink-0 text-white/30 transition-transform ${collapsed.has(category) ? "" : "rotate-90"}`}
            >
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <div className="prose prose-invert prose-sm max-w-none
        ...
        prose-code:bg-white/[0.06] prose-code:px-1.5 prose-code:py-0.5 prose-code:rounded
        prose-pre:bg-[#141414] prose-pre:border prose-pre:border-white/[0.06] prose-pre:rounded-lg
        prose-a:text-blue-400 prose-a:no-underline hover:prose-a:underline
        prose-table:text-[13px] prose-th:text-white/60 prose-th:font-mono prose-th:text-[11px]
        prose-td:text-white/70 prose-td:border-white/[0.06] prose-th:border-white/[0.06]
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-[#0A0A0A]">
        <div className="flex flex-col items-center gap-3">
          ...
          <span className="font-mono text-[12px] text-white/40">Loading portal...</span>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-[#0A0A0A]">
        <div className="flex flex-col items-center gap-4 text-center">
          ...
          <Link
            href="/"
            className="font-mono text-[12px] text-white/30 hover:text-white/60 transition-colors"
          >
            Go to Meter
          </Link>
        </div>
  ...
  return (
    <div className="flex h-screen bg-[#0A0A0A] text-white">
      ...
      <aside
        ...
        } shrink-0 border-r border-white/[0.06] transition-all duration-200 overflow-hidden flex flex-col`}
      >
        <div className="p-4 pb-3 border-b border-white/[0.06] flex items-center gap-3">
          <Link href="/">
            <Image
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex items-center gap-3 px-4 py-2.5 border-b border-white/[0.06] shrink-0">
          ...
          {activeDoc && (
            <span className="font-mono text-[12px] text-white/50 truncate">
              {activeDoc.filePath}
            </span>
          )}
        </div>
          ...
          {activeDoc ? (
            ...
            <div className="flex h-full items-center justify-center">
              <div className="text-center">
                <div className="font-mono text-[13px] text-white/30 mb-2">
                  No documents yet
                </div>
