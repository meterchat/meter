...
export function CardSwitcher() {
  ...
  const cardContent = (
    <>
      <div className="font-mono text-[10px] text-muted-foreground/60 uppercase tracking-wider px-2 py-1">Cards</div>
      {cardsLoading && cards.length === 0 ? (
        <div className="px-2 py-2 font-mono text-[10px] text-muted-foreground/40">Loading cards...</div>
      ) : cards.length === 0 ? (
        <div className="px-2 py-2 font-mono text-[10px] text-muted-foreground/40">No cards on file</div>
      ) : (
        <div className="space-y-1">
          {cards.map((card) => {
            const brandLabel = card.brand.charAt(0).toUpperCase() + card.brand.slice(1);
            return (
              <button key={card.id} onClick={() => { if (!card.isDefault) { trackCardDefaultChanged({ cardId: card.id }); setDefaultCard(card.id); } }}
                className={`flex w-full items-center gap-2 rounded-md px-2 py-1.5 font-mono text-[11px] transition-colors ${card.isDefault ? "bg-foreground/10 text-foreground" : "text-muted-foreground hover:bg-foreground/5 hover:text-foreground"}`}>
                ...
                <span className="ml-auto text-[10px] text-muted-foreground/60">{String(card.expMonth).padStart(2, "0")}/{String(card.expYear).slice(-2)}</span>
              </button>
            );
          })}
        </div>
  ...
  return (
    <div ref={ref} className="relative ml-auto">
      <button
        ...
        className={`flex items-center gap-1 font-mono text-[11px] transition-colors ${displayLast4 ? "text-muted-foreground hover:text-foreground" : "text-muted-foreground/30 hover:text-muted-foreground/60"}`}
      >
