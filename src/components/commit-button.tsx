...
export function CommitButton() {
  ...
  const commitContent = !hasChanges ? (
    <div className="px-4 py-6 text-center">
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="mx-auto mb-2 text-muted-foreground/20">
        <circle cx="12" cy="12" r="4" /><line x1="1.05" y1="12" x2="7" y2="12" /><line x1="17.01" y1="12" x2="22.96" y2="12" />
      </svg>
      <p className="font-mono text-[12px] text-muted-foreground/50">Nothing to commit</p>
      <p className="mt-1 font-mono text-[10px] text-muted-foreground/30">Decisions will appear here as you chat</p>
    </div>
  ) : (
    <>
      {stagedDecisions.length > 0 && (
        <div className="border-b border-border/50 px-4 py-3">
          <div className="font-mono text-[11px] uppercase tracking-wider text-muted-foreground/60 mb-2">{stagedDecisions.length} decision{stagedDecisions.length !== 1 ? "s" : ""} staged</div>
          <div className="flex flex-col gap-1">
            {stagedDecisions.map((d) => (
              <div key={d.id} className="group flex items-start gap-2 rounded-md px-2 py-1.5 hover:bg-foreground/[0.03] transition-colors">
                <span className="mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full bg-amber-500" />
                <div className="min-w-0 flex-1">
                  <div className="font-mono text-[12px] text-foreground/80 truncate">{d.title}</div>
                  {d.choice && <div className="font-mono text-[10px] text-muted-foreground/50 truncate mt-0.5">{d.choice}</div>}
                </div>
                <button onClick={() => handleUnstage(d.id)} className="shrink-0 mt-0.5 rounded p-0.5 text-muted-foreground/30 opacity-0 group-hover:opacity-100 hover:text-foreground hover:bg-foreground/10 transition-all" title="Unstage">
      ...
      <div className="px-4 py-3">
        <div className="font-mono text-[11px] uppercase tracking-wider text-muted-foreground/60 mb-2">Documents</div>
        <div className="flex flex-col gap-0.5">
          {modifiedArtifacts.map((a) => (
            <div key={a.filePath} className="flex items-center gap-2 py-1">
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="shrink-0 text-muted-foreground/50"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" /><polyline points="14 2 14 8 20 8" /></svg>
              <span className="flex-1 font-mono text-[12px] text-foreground/80">{a.filePath}</span>
              <span className={`font-mono text-[10px] ${a.status === "new" ? "text-emerald-500" : "text-amber-500"}`}>{a.status === "new" ? "+new" : "modified"}</span>
            </div>
          ))}
          {unchangedDocuments.map((a) => (
            <div key={a.filePath} className="flex items-center gap-2 py-1 opacity-40">
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="shrink-0 text-muted-foreground/50"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" /><polyline points="14 2 14 8 20 8" /></svg>
              <span className="flex-1 font-mono text-[12px] text-muted-foreground">{a.filePath}</span>
              <span className="font-mono text-[10px] text-muted-foreground/50">no changes</span>
            </div>
          ))}
          {modifiedArtifacts.length === 0 && unchangedDocuments.length === 0 && (
            <div className="py-1 font-mono text-[11px] text-muted-foreground/30">No blueprint files yet</div>
          )}
        </div>
      </div>
      <div className="border-t border-border/50 px-4 py-3">
        <button onClick={handleCommit} disabled={committing} className="flex w-full items-center justify-center gap-2 rounded-lg bg-foreground py-2 font-mono text-[11px] text-background transition-colors hover:bg-foreground/90 active:bg-foreground/80 disabled:opacity-70">
          {committing ? (
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      {open && !isMobile && (
        <div className="absolute top-full z-50 mt-2 rounded-xl border border-border bg-card shadow-xl right-0 w-[340px]">
          {commitContent}
        </div>
      )}
